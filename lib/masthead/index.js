...
module.exports = function createMastheadString(emailType, isFile, addToFilename) {
    ...
    if(config.newsletterType[emailType].originDate) {
      ...
      if(isFile === false) {
        ...
        tagline = [emailType, addToFilename, today.format("MMMYYYY").toLowerCase(), "vol"+volNumber.toString()].filter(function(e){return e});
      ...
      if(isFile === false) {
        ...
        tagline = [emailType, addToFilename, today.format("MMMYYYY").toLowerCase()].filter(function(e){return e});
        tagline = tagline.join('_');
        }
    }
    return tagline;
